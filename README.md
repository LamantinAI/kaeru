...
- **Check memory before asking** (Claude Code and Codex): `contrib/hooks/kaeru-first/` is a harness hook for the moment the agent is about to ask the user something — it recognises the ask even without a question mark, searches kaeru itself, and blocks once with the hits rather than a recipe, then asks the agent to capture the answer. Stdlib Python, fails open, English and Russian lexicons. See its README.
  - An **optional** semantic judge (TypeSafe's Jev) can decide whether a hit actually answers the question, instead of the hook showing three candidates and leaving relevance to the agent. It is **off unless you set a key**, it is not wired into the hook yet, and it has to beat the lexical baseline on a measured corpus first — `contrib/hooks/kaeru-first/eval_judge.py` is that measurement. Enabling it sends the agent's question and the matching nodes' excerpts to a third party, so it is a per-vault decision, never a default.
