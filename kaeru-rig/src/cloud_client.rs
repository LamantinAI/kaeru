...
//!
//! Mirror of the daemon's client (`kaeru-mcp/src/cloud_client.rs`): `kaeru-rig`
//! embeds `kaeru-core` directly for local memory, and reaches the cloud over
//! this same client so a rig agent can `share` / `pull` team knowledge in
//! process — no separate daemon required. Methods return `(status_code,
//! body_text)` and leave JSON parsing to the caller (`cloud`), keeping the
//! client dumb. Bearer auth is sent on every request; an empty token still
//! sends `Bearer ` (the cloud treats an empty *expected* token as auth-disabled).
...
/// black-holed connection blocks the calling tool indefinitely — reqwest
/// sets no total-request timeout by default.
...
/// Named clouds this memory can reach, plus which one is the default.
///
/// Multi-cloud support: one embedded memory may reach several `kaeru-cloud`
/// endpoints (e.g. `family`, `work`). Cloud tools resolve a client by explicit
/// `cloud` name, falling back to the default (or the sole configured cloud).
/// Soft links remember their cloud by name (`dst_store = cloud:<name>`);
/// [`Self::get`] with that parsed name routes resolution back to the right
/// endpoint.
