...
///
/// `layer` defaults to `warm`. The node's `visibility` is stored as given;
/// a node ingested into the shared cloud is typically `Shared`.
...
pub fn upsert_node(
    ...
    let script = format!(
        ...
        ?[id, validity, type, tier, name, body, tags, initiatives, properties, visibility, layer] <-
            [[$id, [{now_secs}.0, true], '{ty}', '{tier}', $name, $body, {tags_lit}, null, null, '{vis}', 'warm']]
